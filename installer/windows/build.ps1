...
if ($wixVersion -notmatch '^4\.') { throw "WiX v4 is required; found $wixVersion" }
...
try {
    ...
    if (-not $signed -and -not $AllowUnsigned) {
        throw 'A signing certificate thumbprint is required. Use -AllowUnsigned only for local installer testing.'
    }
    if ($signed) {
    ...
    & $wix build (Join-Path $scriptDirectory 'Package.wxs') -arch x64 -d "AgentVersion=$AgentVersion" -d "PayloadDir=$stagedPayload" -d "LauncherPath=$launcher" -o $artifact
    ...
    if ($signed) {
        ...
        if ((Get-AuthenticodeSignature -LiteralPath $artifact).Status -ne 'Valid') { throw 'MSI signature validation failed.' }
    }
