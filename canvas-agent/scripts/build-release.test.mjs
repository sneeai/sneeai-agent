...
import { archiveInvocation, bunBuildEnvironment, createReleaseManifest, createReleasePlan, isManagedReleaseFile, npmPackArguments, publishRelease } from "./build-release.mjs";
...
test("release commands require offline Codex packages and metadata-clean archives", () => {
    ...
    const invocation = archiveInvocation(plan.targets[0], "/tmp/stage", "/tmp/stage/bundle", "/tmp/release.zip");
    assert.equal(invocation.command, "tar");
    assert.ok(invocation.args.includes("zip:compression=deflate"));
    assert.ok(invocation.args.includes("--no-xattrs"));
